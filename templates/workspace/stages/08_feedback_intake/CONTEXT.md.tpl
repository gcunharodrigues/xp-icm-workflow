...
| 5 | {{PROJECT_ROOT}}/workspaces/{{WORKSPACE}}/stages/02_design/output/ | L4 | conditional: exists if design ran (v4.0: includes wave-plan.md from inline wave-planner) |
| 6 | {{PROJECT_ROOT}}/workspaces/{{WORKSPACE}}/stages/04_implementation_waves/output/ | L4 | yes (pre-condition: implementation must have run) |
| 7 | {{PROJECT_ROOT}}/.icm-main/docs/lessons.md | L3 | conditional: append only in output A |
| 8 | {{LOGS_ROOT}} | L3 | conditional: sample of last 30 days if L0 declares `logs_root` ≠ null |
| 9 | {{PROJECT_ROOT}}/workspaces/{{WORKSPACE}}/_references/runtime/feedback-intake-stage08.md | L3 | yes (literal protocol) |
| 10 | {{PROJECT_ROOT}}/workspaces/{{WORKSPACE}}/stages/08_feedback_intake/_kickoff.md | L4-kickoff | conditional: legacy file from v3 workspaces. v4.0: human triggers 08 manually after real use — no prior session generates kickoff. |
...
| 12 | {{PROJECT_ROOT}}/workspaces/{{WORKSPACE}}/_references/runtime/recovery-wizard.md | L3 | conditional: referenced in pre-flight if workspace is inconsistent |
...
[n] cancel stage 08 (status returns to BLOCKED with block_reason=human_gate)
...
5. Persistent failure / human cancellation → stop point #15 `runtime_cleanup_failed`. Status `BLOCKED`, `block_reason: stop_point`.
...
1. **Pre-flight — mandatory pre-condition:** L1 must declare `stage_atual: "08"` with `sub_stage: 08_in_progress` (transition from stage 04 last wave) and `status: IN_PROGRESS`. If status is `BLOCKED` with `block_reason = human_gate` → human already approved → set `status = IN_PROGRESS`, remove `block_reason`, proceed. If `stage_atual ≠ 08` → refuse. If status undefined → stop point `workspace_corrupt`.
   - **CWD and branch:** `cd {{PROJECT_ROOT}}` (all relative paths resolve from here). `git branch --show-current` MUST show `workspace/{{WORKSPACE}}`. Wrong branch → `status: BLOCKED, block_reason: error`.
...
2. **Set `status: IN_PROGRESS`** in L1 if currently `BLOCKED`. Append `history` event `feedback_session_started`.
...
5. **Intent inference (session decides A/B/C autonomously):** apply heuristics below. If multiple categories match, priority: B > C > A (point correction beats pivot; pivot beats close). If ambiguous, session asks short clarification before proceeding.
   ...
   |---|---|---|
   | "bug in X", "breaks", "error", "doesn't work", "regression", "fail", "crashed" | **B** restart | Map stage of bug: code/test bug → 04 (implementation), bad design → 02 (design), wrong requirements → 01 (discovery) |
...
**Output B validation:** session refuses X ∉ {`01`, `02`, `04`}. To change `project_root` or project type, use output C.
...
- `BLOCKED` — requires `block_reason`. Values: `human_gate` (runtime cleanup checklist awaiting human), `stop_point` (A/B/C menu for runtime_cleanup_failed or workspace_corrupt), `error` (pre-flight failure, disk full, pre-commit hook rejects).
...
| Bug type | X inferred |
|---|---|
| Test / CI failure / automated regression | 04 implementation_waves (re-run with fix) |
| Runtime bug / broken code / wrong logic | 04 implementation_waves |
| Code smell, bad design that passed review | 02 design (revise plan/ADRs) |
| Wrong plan / inadequate architecture / wrong ADR | 02 design |
| Wrong wave plan / bad DAG / poorly mapped dependencies | 02 design (wave-planner runs inline) |
| Missing requirements / shallow discovery / missed question | 01 discovery |
| Merge with problem / push to wrong branch / wrong tag | 04 implementation_waves (re-merge) |
...
[n] cancel, ask again
[edit] describe the adjustment (ex: "actually restart stage 04, not 02")
```
...
6. **(v3.7.2) Opt-in cleanup menu (only if exit=2):** BEFORE `/init`, offer complete ICM-ephemeral state cleanup. Print literal:
...
7. **(v3.7.0) Auto-invoke `/init` if was last active:** if exit code from step 5 = `2`, invoke `Skill(skill: "init")` AFTER cleanup (step 6). Cleanup runs first so /init output persists on the base branch. If cleanup was skipped (`[n]`), /init still runs — the workspace branch still holds the active CLAUDE.md. If exit = `0`, **DO NOT invoke** `/init` — other workspaces still active, ICM region populated, `/init` would overwrite signaling.

   **IMPORTANT:** After /init completes, persist the regenerated CLAUDE.md to base branch:
   ```bash
   cp {{PROJECT_ROOT}}/CLAUDE.md {{PROJECT_ROOT}}/.icm-main/CLAUDE.md
   cd {{PROJECT_ROOT}}/.icm-main && git add CLAUDE.md && git commit -m "chore: /init regenerated CLAUDE.md after workspace {{WORKSPACE}} close"
   ```
...
1. Validate `X ∈ {01, 02, 04}` (refuse `00` and `08`). Stages 03/05/06/07 no longer exist as separate stages in v4.0 — they were merged into 02 and 04.
...
6. **(v3.7.2) Opt-in cleanup menu (only if exit=2):** BEFORE `/init`, offer complete ICM-ephemeral state cleanup (delete workspace branch, remove `.icm-main/` worktree, clean orphan subagent worktrees). Print literal:
