...
Runs the deterministic wave-planner on top of the `plan.md` from stage 02 design. Builds a DAG from explicit dependencies and file footprints, partitions tasks into topological waves respecting the subagent cap per tier (2/3/5/5), and writes `wave-plan.md` with the list of branches to create for stage 04. Branch format: `wave-{{WORKSPACE_NUM}}-<N>/<task-slug>` (numeric workspace prefix, wave number, task slug). Generated by `wave-planner-script.py`. This stage is deterministic: cycles in the plan become `BLOCKED_ERROR`, not a stop point.
...
Note: `.icm-main/` is the base-branch linked worktree (skill infrastructure). Workspace data lives under `workspaces/{{WORKSPACE}}/`. They are separate git worktrees.
...
- `output/wave-plan.md` — frontmatter (cap, total_tasks, total_waves, llm_review status) + table per sub-wave (slug, files touched, depends on, branch to create) + audit (serialized file conflicts, ambiguities). Each branch follows the format `wave-{{WORKSPACE_NUM}}-<N>/<task-slug>` (numeric workspace prefix, wave number, task slug).
